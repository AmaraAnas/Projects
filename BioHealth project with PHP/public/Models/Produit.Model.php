...
class Produit extends Model
{
    ...
    public function listProduitNew()
    {
        $tab = array();
        $rqt = mysql_query("SELECT * FROM produit WHERE en_new=1");
        while($data = mysql_fetch_assoc($rqt))
            $tab[] = $data;
        return $tab;
    }

    public function listProduitTop()
    {
        $tab = array();
        $rqt = mysql_query("SELECT * FROM produit WHERE en_top=1");
        while($data = mysql_fetch_assoc($rqt))
            $tab[] = $data;
        return $tab;
    }

    public function listProduitVendette()
    {
        $tab = array();
        $rqt = mysql_query("SELECT * FROM produit WHERE en_vendette=1");
        while($data = mysql_fetch_assoc($rqt))
            $tab[] = $data;
        return $tab;
    }
